...
void loop(){     
    ...
    // configure the LED indicators to turn on when pushbutton is pressed or otherwise if not being pressed
      ...
      if (input2 == HIGH)
        ...
        /// code logic of a NOT GATE , use only input1 ///

if (input1 == HIGH)  { // check if the button state is LOW if yes then turn the LED on
digitalWrite(outputPin, LOW); 
 }
 
else 
digitalWrite(outputPin, HIGH); // else if the button state is HIGH turn the LED off
